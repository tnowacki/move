...
fn pack(verifier: &mut ReferenceSafetyAnalysis, num_elements: usize) {
    let stack_len = verifier.stack.len();
    // subtraction safe due to stack safety check
    verifier.stack.truncate(stack_len - num_elements);
    verifier.stack.push(AbstractValue::NonReference)
}

fn unpack(verifier: &mut ReferenceSafetyAnalysis, num_elements: usize) {
    let elems = verifier.stack.pop().unwrap();
    checked_verify!(elems.is_value());
    let stack_len = verifier.stack.len();
    verifier
        .stack
        .resize(stack_len + num_elements, AbstractValue::NonReference);
}
...
fn execute_inner(
    ...
    match bytecode {
        ...
        Bytecode::Pack(idx) => {
            ...
            pack(verifier, num_fields(struct_def))
        }
        ...
        Bytecode::Unpack(idx) => {
            let struct_def = verifier.resolver.struct_def_at(*idx)?;
            unpack(verifier, num_fields(struct_def))
        }
        ...
        Bytecode::VecPack(_, n) => pack(verifier, *n as usize),
        Bytecode::VecUnpack(_, n) => unpack(verifier, *n as usize),
        Bytecode::VecLen(_) => {
            state.release_value(verifier.stack.pop().unwrap());
            verifier.stack.push(AbstractValue::NonReference);
        }
        Bytecode::VecImmBorrow(_) => {
            checked_verify!(verifier.stack.pop().unwrap().is_value());
            let id = verifier.stack.pop().unwrap().ref_id().unwrap();
            let value = state.vector_element_borrow(offset, false, id)?;
            verifier.stack.push(value)
        }
        Bytecode::VecMutBorrow(_) => {
            checked_verify!(verifier.stack.pop().unwrap().is_value());
            let id = verifier.stack.pop().unwrap().ref_id().unwrap();
            let value = state.vector_element_borrow(offset, true, id)?;
            verifier.stack.push(value)
        }
        Bytecode::VecPushBack(_) => {
            checked_verify!(verifier.stack.pop().unwrap().is_value());
            let id = verifier.stack.pop().unwrap().ref_id().unwrap();
            state.vector_mut_op(offset, id)?;
        }
