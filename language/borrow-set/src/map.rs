...
pub(crate) struct BorrowMap<Loc: Copy, Instr: Copy + Ord, Lbl: Clone + Ord> {
    ...
    BorrowMap<Loc, Instr, Lbl>
{
    ...
    pub(crate) fn consistent_world(&self, other: &Self) -> bool {
        // same keys
        self.map.keys().all(|id| other.map.contains_key(id))
            ...
            && self.map.keys().all(|id| {
                self.map[id].is_pinned() == other.map[id].is_pinned()
            ...
            && self
                ...
                .all(|id| self.is_pinned_released(*id) == other.is_pinned_released(*id))
    }
